.PHONY: help install dev-install lint format typecheck security vulnerabilities complexity licenses \
...
complexity:  ## Check cyclomatic complexity (ruff C90 rules)
	...
	ruff check --select C90,PLR --ignore PLR0913 src

vulnerabilities:  ## Check for known vulnerabilities in dependencies
	pip-audit --strict || echo "⚠️  Vulnerability check found issues (non-blocking)"
