...
> **Minimum version: b8665** — required for native Gemma 4 tool calling.
> See [`llamacpp_b8665_upgrade.md`](llamacpp_b8665_upgrade.md) for build steps.
...
SEMANTIC_LLAMACPP_MAX_CONTEXT_LENGTH=524288 \
SEMANTIC_LLAMACPP_N_SLOTS=4 \
SEMANTIC_SERVER_SEARXNG_URL=http://localhost:8080 \
python -m uvicorn agent_memory.entrypoints.api_server:create_app \
    --factory --host 0.0.0.0 --port 8000
...
| DNS `main4.local` not resolving | mDNS not working on Mac | Use IP directly: `ssh <user>@<thor-hostname> "hostname -I"` |
...
ssh <user>@<thor-hostname> "ss -tlnp | grep 8000"

# Is agent-memory running?
ssh <user>@<thor-hostname> "ps aux | grep uvicorn | grep -v grep"

# Is llama-server running?
ssh <user>@<thor-hostname> "ps aux | grep llama-server | grep -v grep"

# Slot cache files on Thor
ssh <user>@<thor-hostname> "ls -lh ~/.agent_memory/llamacpp_slots/"
```
...
| `ANTHROPIC_MODEL` | `gemma-4-26b-a4b` | project `.claude/settings.json` |
...
| `SEMANTIC_LLAMACPP_MODEL_ID` | `gemma-4-26b-a4b` | Thor, agent-memory startup |
