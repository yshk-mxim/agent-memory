...
KNOWN_TOOLS: dict[str, str] = {
    ...
    "WebFetch": (
        "Fetch URL content. "
        "Params: url (str, required), prompt (str, required)"
    ),
    "AskUserQuestion": (
        "Ask user a question. "
        "Params: question (str, required)"
    ),
    "NotebookEdit": (
    ...
    "Skill": (
        ...
        "Params: skill (str, required), args (str)"
    ),
    "ToolSearch": (
        "Fetch deferred tool schemas. "
        "Params: query (str, required), max_results (int)"
    ),
}
