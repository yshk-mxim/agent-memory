...
\addlegendentry{User B wait (submit $\to$ first token)}
...
\end{axis}
\end{tikzpicture}
\caption{Staggered request arrivals (4K cold context, User B arrives 2s after User A). Total wall time is similar between sequential and batched modes for both models. User B benefits from batched scheduling because it begins prefill immediately rather than waiting for User A's decode phase to complete. The effect is larger on DeepSeek (1.36$\times$) due to shorter prefill relative to decode.}
\label{fig:staggered}
\end{figure}
