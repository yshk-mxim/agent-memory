...
For Gemma, total wall time is similar (38.8\,s sequential vs 38.6\,s batched). For DeepSeek, also similar (9.5\,s vs 9.4\,s). The benefit of batching appears in User~B's time-to-completion: DeepSeek batched User~B finishes in 6.4\,s vs 8.7\,s sequential from experiment start (includes both queuing and processing time). The effect is smaller for Gemma because its prefill is long enough to dominate. With warm or hot caches, the staggered benefit would be larger since prefill overhead vanishes and decode interleaving matters more.
