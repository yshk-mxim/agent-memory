...
Latency hiding follows from multi-agent structure. In a 5-agent round-robin, while Agent~A generates (1--3\,s for 50--100 tokens), Agent~B's cache loads from disk (${\sim}$500\,ms at 7\,GB/s). The interleaved scheduler already implements this for prefill chunks. In principle, only $1/N$ of cold-start latency falls on the critical path, where $N$ is the number of active agents. For $N{=}5$, cache reload could run concurrently with generation 80\% of the time. This $1/N$ projection has not been measured end-to-end; the staggered arrival results (Appendix~\ref{app:staggered}) provide partial validation for $N{=}2$.
