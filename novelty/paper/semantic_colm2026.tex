...
\textbf{Memory bandwidth convergence.} The benchmark hardware (Apple M4 Pro, model MX2E3LL/A) provides 273 GB/s memory bandwidth~\cite{apple2024m4pro}. NVIDIA's DGX Spark (October 2025, \$3,999, 128 GB unified memory~\cite{nvidia2025dgxspark}), marketed as a ``personal AI supercomputer'' workstation, provides the same 273 GB/s bandwidth. This convergence suggests unified memory architectures are becoming competitive across edge and workstation devices. However, traditional datacenter accelerators still dominate in compute throughput (10--50$\times$ higher prefill speeds) and total memory capacity.
...
\midrule
Warm speedup & 1.9$\times$ & 2.9$\times$ & 4.2$\times$ & 4.2$\times$ & 10.5$\times$ & 10.5$\times$ \\
Hot speedup & 2.7$\times$ & 5.0$\times$ & 9.3$\times$ & 17.3$\times$ & 81.6$\times$ & 147$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Contributions and Comparison with Related Systems}

This work addresses a distinct problem in KV cache management: per-agent persistent Q4 storage on edge devices with batched quantized inference and working memory semantics. Table~\ref{tab:novelty} compares our system with related work across five key capabilities:
