...
def _make_request(agent_id="test", n_tokens=100, cache=None, loop=None):
    if loop is None:
...
class TestProperties:
    ...
    def test_pending_prefills_count(self):
        sched = _make_scheduler()
    ...
    def test_active_decodes_count(self):
        sched = _make_scheduler()
        assert sched.active_decodes == 0
        sched._uid_to_request["uid_1"] = _make_request()
        assert sched.active_decodes == 1

    def test_waiting_requests_count(self):
        sched = _make_scheduler()
        assert sched.waiting_requests == 0
        sched._waiting_queue.append(_make_request())
...
class TestDispatchRequest:
    def test_warm_cache_goes_direct(self):
        sched = _make_scheduler()
        cache = MagicMock()
        cache.total_tokens = 50
        req = _make_request(n_tokens=100, cache=cache)

        sched._dispatch_request(req)
        sched._engine.submit.assert_called_once()

    def test_cold_short_prompt_goes_direct(self):
        sched = _make_scheduler(interleave_threshold=2048)
        req = _make_request(n_tokens=100)

        sched._dispatch_request(req)
        sched._engine.submit.assert_called_once()

    def test_cold_long_prompt_goes_prefill(self):
        sched = _make_scheduler(interleave_threshold=100)
        req = _make_request(n_tokens=200)
...
class TestPromoteWaiting:
    def test_active_decodes_blocks_promotion(self):
        sched = _make_scheduler()
        sched._uid_to_request["uid_1"] = _make_request()
        sched._waiting_queue.append(_make_request())
    ...
    def test_promotes_up_to_max_batch(self):
        sched = _make_scheduler(max_batch_size=2)
        sched._engine.submit.side_effect = [f"uid_{i}" for i in range(5)]
        for i in range(3):
            sched._waiting_queue.append(_make_request(agent_id=f"a{i}", n_tokens=10))
...
class TestSubmitDirectErrors:
    def test_pool_exhausted_rejects(self):
        engine = _make_engine()
    ...
    def test_invalid_request_rejects(self):
        engine = _make_engine()
...
class TestEnqueuePrefill:
    def test_creates_prefill_state(self):
        sched = _make_scheduler()
        req = _make_request(n_tokens=5000)
...
class TestRunDecodeStep:
    def test_in_progress_token_pushed_to_queue(self):
        engine = _make_engine()
        sched = _make_scheduler(engine=engine)
    ...
    def test_finish_reason_resolves(self):
        engine = _make_engine()
        sched = _make_scheduler(engine=engine)
    ...
    def test_exception_rejects_all(self):
        engine = _make_engine()
...
class TestProcessOneChunk:
    def test_normal_chunk_advances(self):
        sched = _make_scheduler()
    ...
    def test_prefill_complete_promotes(self):
        engine = _make_engine()
    ...
    def test_exception_during_prefill(self):
        sched = _make_scheduler()
...
class TestFutureHelpers:
    ...
    def test_reject_request_with_queue(self):
        sched = _make_scheduler()
        loop = asyncio.new_event_loop()
        req = _make_request(loop=loop)
        req.token_queue = asyncio.Queue()
