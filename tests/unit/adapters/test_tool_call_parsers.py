...
class TestGemmaParser:
    ...
    # ── Gemma 4 <|"|> string delimiter tests (matching llama.cpp test cases) ──

    def test_gemma_delim_simple_string(self):
        text = 'call:get_weather{city:<|"|>London<|"|>}'
        _, calls = self.parser.parse(text)
        assert len(calls) == 1
        assert calls[0].input["city"] == "London"

    def test_gemma_delim_string_with_spaces(self):
        text = 'call:get_weather{city:<|"|>San Francisco<|"|>}'
        _, calls = self.parser.parse(text)
        assert calls[0].input["city"] == "San Francisco"

    def test_gemma_delim_with_inner_quotes(self):
        """Content with double-quotes inside <|"|> delimiters must be escaped."""
        text = 'call:Write{content:<|"|>She said "hello" to me<|"|>}'
        _, calls = self.parser.parse(text)
        assert len(calls) == 1
        assert 'She said "hello" to me' == calls[0].input["content"]

    def test_gemma_delim_with_inner_braces(self):
        """Braces inside <|"|> strings must not confuse brace depth tracking."""
        text = 'call:Write{content:<|"|>fn() { return 1; }<|"|>}'
        _, calls = self.parser.parse(text)
        assert len(calls) == 1
        assert "{ return 1; }" in calls[0].input["content"]

    def test_gemma_delim_integer_value(self):
        text = "call:SetLimit{count:42}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["count"] == 42

    def test_gemma_delim_negative_number(self):
        text = "call:SetOffset{offset:-7}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["offset"] == -7

    def test_gemma_delim_float(self):
        text = "call:SetPi{value:3.14}"
        _, calls = self.parser.parse(text)
        assert abs(calls[0].input["value"] - 3.14) < 0.001

    def test_gemma_delim_boolean(self):
        text = "call:Toggle{enabled:true,disabled:false}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["enabled"] is True
        assert calls[0].input["disabled"] is False

    def test_gemma_delim_null(self):
        text = "call:Clear{value:null}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["value"] is None

    def test_gemma_delim_array(self):
        text = 'call:AddTodos{todos:[<|"|>buy milk<|"|>,<|"|>walk dog<|"|>]}'
        _, calls = self.parser.parse(text)
        assert calls[0].input["todos"] == ["buy milk", "walk dog"]

    def test_gemma_delim_nested_dict(self):
        text = 'call:Configure{config:{theme:<|"|>dark<|"|>,count:3}}'
        _, calls = self.parser.parse(text)
        assert calls[0].input["config"]["theme"] == "dark"
        assert calls[0].input["config"]["count"] == 3

    def test_gemma_delim_empty_array(self):
        text = "call:Clear{items:[]}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["items"] == []

    def test_gemma_delim_empty_dict(self):
        text = "call:Reset{config:{}}"
        _, calls = self.parser.parse(text)
        assert calls[0].input["config"] == {}
