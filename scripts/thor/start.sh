...
export SEMANTIC_LLAMACPP_TIMEOUT_S=120
export SEMANTIC_LLAMACPP_AUTO_SWAP=true
...
if [ ! -x "$LLAMA_SERVER" ]; then
    ...
    echo "Build it: cd ~/llama.cpp-build && cmake -B build -DGGML_CUDA=ON -DCMAKE_CUDA_ARCHITECTURES=110 . && cmake --build build -j\$(nproc) --target llama-server"
    exit 1
fi
